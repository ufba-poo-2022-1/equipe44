...
public class CriaJogadorV2 implements ActionListener {
	...
	public String passaNumero() {
		if (Main.getSumula().visitante.toString().contains(numText.getText())) {
			for (Jogador j : Main.getSumula().visitante.jogadores) {
		...
		Main.getSumula().visitante.jogadores.add(j);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == confirma) {
			...
			Main.getSumula().esquerda.remove(nomeLabel);
			Main.getSumula().esquerda.validate();
			Main.getSumula().esquerda.repaint();

			Main.getSumula().visitante2.setText(passaNumero());

			nomeLabel.setBounds(200, 300, 230, 70);
			nomeLabel.setText(passaNome());
			nomeLabel.setFocusable(false);
			nomeLabel.setFont(new Font("Verdana", Font.PLAIN, 25));

			pontuacao.setBounds(460, 300, 70, 70);
			pontuacao.setText(passaPontos());
			pontuacao.setFont(new Font("Verdana", Font.PLAIN, 25));

			faltas.setBounds(560, 300, 70, 70);
			faltas.setText(passaFaltas());
			faltas.setFont(new Font("Verdana", Font.PLAIN, 25));

			sub.setBounds(660, 300, 90, 70);
			sub.setFont(new Font("Verdana", Font.PLAIN, 25));
			sub.addActionListener(this);

			Main.getSumula().esquerda.add(nomeLabel);
			Main.getSumula().esquerda.add(pontuacao);
			Main.getSumula().esquerda.add(faltas);
			Main.getSumula().esquerda.add(sub);
			Main.getSumula().esquerda.validate();
			Main.getSumula().esquerda.repaint();
		...
		if (e.getSource() == sub) {
			if (Main.getSumula().visitante.toString().contains(numText.getText())) {
				for (Jogador j : Main.getSumula().visitante.jogadores) {
					if (j.numero.equals(Main.getSumula().visitante2.getText())) {
			...
			Main.getSumula().visitante2.setText("#0");
